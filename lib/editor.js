            ...
            for (var key in postData) {
                if (key === 'files' || key === 'images') {
                    ...
                    temp.forEach(function(ele) {
                        ele.url = window.location.protocol + "//" + window.location.hostname + ele.url
                    });
                    postData[key] = JSON.stringify(temp);
                }
            }
        ...
        var uploadHandler = function (callback) {
            return function (e) {
                ...
                if (file) {
                    ...
                    formData.append('cid', $form.attr('data-cid'));
                ...
                switch (ele.type) {
                    case 'text-count':
                        if (obj[ele.key] === undefined || obj[ele.key].length > ele.input[1] || obj[ele.key].length < ele.input[0]) {
                            ...
                            app.alertError(LANG['error_title']);
                        }
                        break;
                    case 'file':
                    case 'image':
