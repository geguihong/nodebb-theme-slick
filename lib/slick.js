...
$(document).ready(function () {
	...
	$(window).on('action:ajaxify.end', function(ev, data) {
		window.gtag_pageview && window.gtag_pageview();
		
		if (data.url.indexOf('login') === 0 && data.url.indexOf('original=1') === -1) {
			window.location.href = '/auth/arashivision';
		}
	});

	$(window).on('action:topic.loaded', function(ev, data) {
		window.gtag && window.gtag('event', 'topic_c'+data.cid+'_t'+data.tid, {
